...
void find_semi_moves(int* total_semi_quiet, int* total_semi_captures, Move semi_quiet_moves[256], Move semi_capture_moves[256], struct GameState* Game) {
    ...
    int pInd;
    int target_square;
    int piece_side = Game->side_to_move;
    struct piece* Pieces = Game->side_to_move ? Game->white_pieces : Game->black_pieces;
    ...
    for(int i = 15; i >= 0; i--) {  
        if (Pieces[i].alive) {
            ...
            if (type == aKing) {
                ...
                int short_castle = (Game->side_to_move == SIDE_WHITE) ? WK : BK;
                int long_castle = (Game->side_to_move == SIDE_WHITE) ? WQ : BQ;
                ...
                for (int k = startDf; k < endDf; k++) {
                    ...
                    for (int path_file = file + df; path_file != rookFile[k]; path_file = path_file + df) {
                        if (Game->board[rank][path_file] != EMPTY) {
                            blocked = 1;
                        }
                    }
...
static inline int find_pinned_and_checking_pieces(const struct GameState* Game, int pinned_pieces_square[16], int pinned_rdir[16], int pinned_fdir[16], int checking_pieces_square[16], int* total_pinned_pieces, int* total_checking_pieces, int board[8][8]) {
    ...
    int enemy_sign = (Game->side_to_move == SIDE_WHITE) ? -1 : 1;
...
static inline void generate_pseudo_captures(int* total_semi_captures, Move semi_capture_moves[256], struct GameState* Game, int pinned_pieces_squares[16], int total_pinned_pieces, int pinned_rdirs[16], int pinned_fdirs[16]) {
...
static inline void generate_pseudo_moves(int* total_semi_quiet, int* total_semi_captures, Move semi_quiet_moves[256], Move semi_capture_moves[256], struct GameState* Game, int pinned_pieces_squares[16], int total_pinned_pieces, int pinned_rdirs[16], int pinned_fdirs[16]) {
...
static inline void generate_pseudo_evasions(int* total_semi_quiet, int* total_semi_captures, Move semi_quiet_moves[256], Move semi_capture_moves[256], struct GameState* Game, int pinned_pieces_squares[16], int total_pinned_pieces, int pinned_rdirs[16], int pinned_fdirs[16], int checking_pieces_squares[16], int total_checking_pieces) {
...
int generate_legal_captures(struct GameState* Game, Move legal_moves[256]) {
    ...
    /*
     Finds and adds legal moves to Game->legal_moves.
    ...
    find_pinned_and_checking_pieces(Game, pinned_pieces_square, pinned_rdir, pinned_fdir, checking_pieces_square, &total_pinned_pieces, &total_checking_pieces, Game->board);
    ...
    generate_pseudo_captures(&total_semi_captures, semi_capture_moves, Game, pinned_pieces_square, total_pinned_pieces, pinned_rdir, pinned_fdir);
    for (int SemiMove = 0; SemiMove < total_semi_captures; SemiMove++) {
        ...
        int piece_type = Game->board[square / 8][square % 8];
        ...
        if (validate) {
            ...
            king_square = (Game->side_to_move == SIDE_BLACK) ? Game->white_king_square : Game->black_king_square;
...
int generate_legal_moves(struct GameState* Game, Move legal_moves[256]) {
    ...
    if (total_checking_pieces == 0) {
        generate_pseudo_moves(&total_semi_quiet, &total_semi_captures, semi_quiet_moves, semi_capture_moves, Game, pinned_pieces_square, total_pinned_pieces, pinned_rdir, pinned_fdir);
        ...
        generate_pseudo_evasions(&total_semi_quiet, &total_semi_captures, semi_quiet_moves, semi_capture_moves, Game, pinned_pieces_square, total_pinned_pieces, pinned_rdir, pinned_fdir, checking_pieces_square, total_checking_pieces);
...
int is_move_legal(struct GameState* Game, char input[6], uint32_t* matched_legal_move) {
