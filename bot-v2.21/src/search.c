...
static int quiescence_search(struct GameState* Game, struct NodeState Node, struct SearchContext* Search, int alpha, int beta, int checkBuffer) {
    ...
    if (!inCheck) {
        ...
        /*
        uint64_t inc_white_pawns = Game->eval.white_pawns;
        uint64_t inc_black_pawns = Game->eval.black_pawns;

        initiate_evaluation(Game);

        uint64_t ini_white_pawns = Game->eval.white_pawns;
        uint64_t ini_black_pawns = Game->eval.black_pawns;


        assert(inc_white_pawns == ini_white_pawns);
        assert(inc_black_pawns == ini_black_pawns);
*/
