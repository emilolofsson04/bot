    ...
    for (int i = 0; i < total_legal_moves; i++) {
       ...
       // If capture -> sort with mvv_lva
        if (get_capture_flag(legal_moves[i]) == CAPTURE) {
            evals[i] =  1000000000 + mvv_lva(gs, legal_moves[i], *last_played_move);
        }
...
void evaluate_moves(struct GameState* gs, uint32_t legal_moves[256], int total_legal_moves, uint32_t* last_played_move, int eval[256], uint32_t old_pv_line[64], int old_pv_length, int on_pv, int ply, uint32_t killer_moves[64][2], int history_table[64][64]) {
    ...
    for (int i = 0; i < total_legal_moves; i++) {
        ...
        // If capture -> sort with mvv_lva
        if (get_capture_flag(legal_moves[i]) == CAPTURE) {
            eval[i] =  1000000000 + mvv_lva(gs, legal_moves[i], *last_played_move);
        }
        else { // else history table and killer moves
            int ss = get_from_square(legal_moves[i]);
            int ts = get_to_square(legal_moves[i]);
            ...
            for (int k = 0; k < 2; k++) {
                if (legal_moves[i] == killer_moves[ply][k]) {
                    ...
                    if (legal_moves[i] == killer_moves[ply - 2][k]) eval[i] += 1000000;
                }
            }
