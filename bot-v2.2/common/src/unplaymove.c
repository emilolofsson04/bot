...
void unplay_move(uint32_t played_move, struct GameState* gs, struct UndoInfo* ui){
    ...
    gs->zobrist_hash_3fold = ui->zobrist_hash_3fold;
...
/*
void unplay_move(uint32_t played_move, struct GameState* gs){


    // Revert castling rights and ep square, and side to move
    (gs->ply)--;
    struct StateInfo* si = &gs->StateInfo[gs->ply];
    gs->castling_rights = si->castling_rights;
    gs->en_passant_square = si->en_passant_square;
    gs->white_king_square = si->white_king_square;
    gs->black_king_square = si->black_king_square;
    gs->halfmove_clock = si->halfmove_clock;
    gs->side_to_move = !gs->side_to_move;
    gs->zobrist_hash_3fold = si->zobrist_hash_3fold;
    int i    = si->captured_piece_index;
    ...
    if (get_capture_flag(played_move) == CAPTURE) {
            captured_piece->alive = 1;
            captured_piece->rank = rank_capture;
            captured_piece->file = target_file;
            gs->Board[rank_capture][target_file] = captured_piece->type;
    }
}
