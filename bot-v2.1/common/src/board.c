...
void read_fen(const char *fen_string, struct GameState* gs) {
    ...
    while (i < fen_length && fen_string[i] != ' ') {
        ...
        if (c == '/') {
            ...
            if (piece > 0) {
                ...
                Piece->colour = 1;
                total_white_pieces++;
                gs->zobrist_hash_3fold ^= zobrist_pieces[piece - 1][rank * 8 + file];
            }
            else {
                ...
                Piece->colour = 0;
                total_black_pieces++;
                gs->zobrist_hash_3fold ^= zobrist_pieces[6 + abs(piece) - 1][rank * 8 + file];
            }
    ...
    if (gs->side_to_move == SIDE_BLACK) {
        gs->zobrist_hash_3fold ^= zobrist_side_to_move;
    }
    ...
    gs->zobrist_hash_3fold ^= zobrist_castling_rights[gs->castling_rights];
    ...
    if (fen_string[i] != '-') {
        ...
        gs->zobrist_hash_3fold ^= zobrist_en_passant_file[ep_file];
        i += 2;  
    } 
    else {
        gs->en_passant_square = -1;
    ...
    gs->zobrist_hash_3fold_history[gs->ply] = gs->zobrist_hash_3fold;
