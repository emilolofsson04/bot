...
static inline void store_tt(struct NodeState Node, struct GameState* Game, Move best_move, int best, int old_alpha, int old_beta) {
...
static int branch(struct GameState* Game, struct NodeState Node, struct SearchContext* Search) {
    ...
    if (Node.depth == 0) {
        int Check_Buffer = 3;
        Node.on_pv = 0;
        return quiescence_search(Game, Node, Search, Check_Buffer);
    }
     (Search->es.nodes)++;
    ...
    int eval[256] = {0};
    evaluate_moves(Game, legal_moves, total_legal_moves, Node, eval, Search, hash_move); 

    Move best_move = {0};
    for (int k = 0; k < total_legal_moves; k++) {
    
        pick_best_move_first(eval, legal_moves, total_legal_moves, k);
        ...
        // Swap colour and continue branching
        struct NodeState child_node = create_child_node(Node, Search, move);
        branch_eval = -branch(Game, child_node, Search);
    ...
    // Store in tt and return best
    store_tt(Node, Game, best_move, best, old_alpha, old_beta);
    return best;
}
