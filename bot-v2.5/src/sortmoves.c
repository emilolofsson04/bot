...
void evaluate_moves(struct GameState* gs, Move legal_moves[256], int total_legal_moves, struct NodeState Node, int eval[256], struct SearchContext* Search, Move hash_move) {
    ...
    for (int i = 0; i < total_legal_moves; i++) {
        ...
        if (Node.on_pv && Search->old_pv_length > Node.ply) {
            if (legal_moves[i] == Search->old_pv_line[Node.ply]) {
                eval[i] += 1000000000;
                pv_moves++;
                assert(pv_moves <= 1);
                continue;
        ...
        if (get_capture_flag(legal_moves[i]) == CAPTURE) {
            ...
            for (int k = 0; k < 2; k++) {
                if (legal_moves[i] == Search->killer_moves[Node.ply][k]) {
                        eval[i] += (2 - k) * 1000000;
                        break;
                }
                else if (Node.ply > 2) {
                    if (legal_moves[i] == Search->killer_moves[Node.ply - 2][k]) eval[i] += 100000;
                }
            }
