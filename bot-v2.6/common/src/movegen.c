...
static inline void register_move(uint32_t legal_moves[256], int* total_moves, int rank, int file, int pInd, int piece_side, int target_rank, int target_file, int capture, int type, int movetype) {


    int idx = *total_moves;
    if (movetype != PROMOTION) {
        uint32_t* m = &legal_moves[idx];
...
void find_semi_moves(int* total_semi_quiet, int* total_semi_captures, uint32_t semi_quiet_moves[256], uint32_t semi_capture_moves[256], struct GameState* gs) {
...
static inline void generate_pseudo_captures(int* total_semi_captures, uint32_t semi_capture_moves[256], struct GameState* gs, int pinned_pieces_squares[16], int total_pinned_pieces, int pinned_rdirs[16], int pinned_fdirs[16]) {
...
static inline void generate_pseudo_moves(int* total_semi_quiet, int* total_semi_captures, uint32_t semi_quiet_moves[256], uint32_t semi_capture_moves[256], struct GameState* gs, int pinned_pieces_squares[16], int total_pinned_pieces, int pinned_rdirs[16], int pinned_fdirs[16]) {
...
static inline void generate_pseudo_evasions(int* total_semi_quiet, int* total_semi_captures, uint32_t semi_quiet_moves[256], uint32_t semi_capture_moves[256], struct GameState* gs, int pinned_pieces_squares[16], int total_pinned_pieces, int pinned_rdirs[16], int pinned_fdirs[16], int checking_pieces_squares[16], int total_checking_pieces) {
...
int generate_legal_captures(struct GameState* gs, uint32_t legal_moves[256]) {
    ...
    uint32_t semi_capture_moves[256];
    ...
    for (int SemiMove = 0; SemiMove < total_semi_captures; SemiMove++) {
        ...
        uint32_t move = semi_capture_moves[SemiMove]; 
...
int generate_legal_moves(struct GameState* gs, uint32_t legal_moves[256]) {
    ...
    if (total_checking_pieces == 0) {
        ...
        for (int SemiMove = 0; SemiMove < total_semi_quiet + total_semi_captures; SemiMove++) {
            ...
            uint32_t move = (SemiMove < total_semi_captures) ? semi_capture_moves[SemiMove] : semi_quiet_moves[SemiMove - total_semi_captures];
