...
static int branch(struct GameState* gs, uint32_t* last_played_move, int ply, int depth, int alpha, int beta, struct EvalBoards* eb, struct EngineStats* es, uint32_t pvTable[64][64], int pvLength[64], uint32_t old_pv_line[64], int old_pv_length, int on_pv, uint32_t killer_moves[64][2], int history_table[64][64]) {
    ...
    int futility_prune = 0;
    int static_evaluation = 0;
    if (depth == 1) {
        static_evaluation = Eval(gs, eb);
    }
     (es->nodes)++;
    ...
    for (int k = 0; k < total_legal_moves; k++) {
        ...
        if (depth == 1 && static_evaluation < alpha - 50 && get_capture_flag(move) == QUIET) continue;
        ...
        if (beta <= alpha) {
            (es->cutoffmove[k])++;
            if (move == hash_move) (es->hash_move_cutoff)++;
    ...
    if (tt_age > 0 || depth >= TT[tt_index].depth) {
        TT[tt_index].zobrist_key = gs->zobrist_hash;
        TT[tt_index].best_move = best_move;
        TT[tt_index].depth = depth;
        TT[tt_index].generation = tt_generation;
        if (best > old_alpha && old_beta > best) {
            TT[tt_index].eval = best;
            TT[tt_index].flag = EXACT;
        }
        else {
            if (best <= old_alpha) {
                TT[tt_index].eval = best;
                TT[tt_index].flag = UPPERBOUND;
            //    TT[tt_index].best_move = 0;

            }
            else if (best >= old_beta) {
                TT[tt_index].eval = best;
                TT[tt_index].flag = LOWERBOUND;
            //    TT[tt_index].best_move = 0;
            }
        }
    }
...
uint32_t iterative_deepening(struct GameState gs, int max_depth, int max_time) {
    ...
    */
    ...
    // Triangular pv table
    uint32_t pvTable[64][64] = {0};
    int pvLength[64] = {0};
    uint32_t old_pv_line[64] = {0};
    int old_pv_length = 0;         
    int on_pv = 1;
