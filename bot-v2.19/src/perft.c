...
static void PerftBranch(struct GameState* Game, int depth, struct EngineStats* es) {
    ...
    */
    ...
    for (int k = 0; k < move_count; k++) {
        ...
        struct UndoInfo ui;
        play_move(legal_moves[k], Game, &ui); 

        PerftBranch(Game, depth - 1, es);

        unplay_move(legal_moves[k], Game, &ui);
    }

    return;
}

static inline void print_move_nodes(uint64_t* old_nodes, Move played_move, struct EngineStats* es) {
    char move_str[6];
    move_to_uci_string(played_move, move_str);
    printf("%s: %lu\n", move_str, es->nodes - *old_nodes);
    *old_nodes = es->nodes;
}

void Perft(struct GameState Game, int depth, struct EngineStats* es) {
    ...
    */
    ...
    struct GameState Game_copy = Game;
    ...
    for (int k = 0; k < move_count; k++) {
        ...
        print_move_nodes(&old_nodes, legal_moves[k], es);
    }

    verify_state_restored(&Game_copy, &Game);
}
