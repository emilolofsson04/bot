...
static inline int mvv_lva(struct GameState* gs, Move move) {
...
void evaluate_moves_quiescence(struct GameState* gs, Move legal_moves[256], int total_legal_moves, int evals[256], int history_table[64][64]) {
    ...
    for (int i = 0; i < total_legal_moves; i++) {
       ...
       // If capture -> sort with mvv_lva
        if (get_capture_flag(legal_moves[i]) == CAPTURE) {
            evals[i] =  100000000 + mvv_lva(gs, legal_moves[i]);
        }
...
void sort_moves_quiescence(struct GameState* gs, Move legal_moves[256], int total_legal_moves, int eval_order[256], int history_table[64][64]) {
...
void evaluate_moves(struct GameState* gs, Move legal_moves[256], int total_legal_moves, struct NodeState Node, int eval[256], struct SearchContext* Search, Move hash_move) {
    ...
    for (int i = 0; i < total_legal_moves; i++) {
        ...
        // If capture -> sort with mvv_lva
        if (get_capture_flag(legal_moves[i]) == CAPTURE) {
            eval[i] +=  10000000 + mvv_lva(gs, legal_moves[i]);
        }
        else { // else history table and killer moves
            int ss = get_from_square(legal_moves[i]);
            int ts = get_to_square(legal_moves[i]);
