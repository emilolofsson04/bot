...
static inline void update_evaluation(struct MoveInfo Move, struct GameState* Game, struct Evalboards* eb, struct piece* moving_piece, struct piece aPieces[16], struct piece ePieces[16], struct UndoInfo* Undo) {
    ...
    if (side_to_move == SIDE_WHITE) {
        int mg_eval_change = eb->EarlyPST[final_piece_type][final_rank][final_file] - eb->EarlyPST[Move.moving_piece_type][Move.start_rank][Move.start_file];
        int eg_eval_change = eb->LatePST[final_piece_type][final_rank][final_file]  - eb->LatePST[Move.moving_piece_type][Move.start_rank][Move.start_file];
        Game->eval.mg_evaluation += mg_eval_change;
        Game->eval.eg_evaluation += eg_eval_change;

        if (Move.moving_piece_type == PAWN) {
            Game->eval.white_pawn_files -= 1u << (4 * Move.start_file);

            if (Move.move_type != PROMOTION) {
                Game->eval.white_pawn_files += 1u << (4 * final_file);
            }
        }
    }
    else {
        int mg_eval_change = eb->EarlyPST[final_piece_type][7 - final_rank][final_file] - eb->EarlyPST[abs(Move.moving_piece_type)][7 - Move.start_rank][Move.start_file];
        int eg_eval_change = eb->LatePST[final_piece_type][7 - final_rank][final_file]  - eb->LatePST[abs(Move.moving_piece_type)][7 - Move.start_rank][Move.start_file];
        Game->eval.mg_evaluation -= mg_eval_change;
        Game->eval.eg_evaluation -= eg_eval_change;

        if (abs(Move.moving_piece_type) == PAWN) {
            Game->eval.black_pawn_files -= 1u << (4 * Move.start_file);

            if (Move.move_type != PROMOTION) {
                Game->eval.black_pawn_files += 1u << (4 * final_file);
            }
        }
    }
    ...
    if (Move.capture_flag == CAPTURE) {
        ...
        if (side_to_move == SIDE_WHITE) {
            ...
            Game->eval.mg_evaluation  += eb->EarlyPST[type][pst_rank][file];
            Game->eval.eg_evaluation  += eb->LatePST[type][pst_rank][file];

            if (type == BISHOP) Game->eval.black_bishops--;

            if (type == PAWN) Game->eval.black_pawn_files -= 1u << (4 * file);
            ...
            if (type == PAWN) Game->eval.white_pawn_files -= 1u << (4 * file);
        }
    }

    if (Move.move_type == PROMOTION) {
        int promo_val = eb->PieceValues[final_piece_type] - eb->PieceValues[PAWN];
        int promo_phase = eb->game_stage_values[final_piece_type];
    ...
    if (Move.move_type == SHORT_CASTLE || Move.move_type == LONG_CASTLE) {
        ...
        if (side_to_move == SIDE_WHITE) {
            int mg_eval_change = eb->EarlyPST[ROOK][Move.target_rank][rook_file] - eb->EarlyPST[ROOK][Move.target_rank][Move.target_file];
            int eg_eval_change = eb->LatePST[ROOK][Move.target_rank][rook_file]  - eb->LatePST[ROOK][Move.target_rank][Move.target_file];
            Game->eval.mg_evaluation += mg_eval_change;
            Game->eval.eg_evaluation += eg_eval_change;
        } 
        else {
            int rook_rank = 7 - Move.target_rank;
            int mg_eval_change = eb->EarlyPST[ROOK][rook_rank][rook_file] - eb->EarlyPST[ROOK][rook_rank][Move.target_file];
            int eg_eval_change = eb->LatePST[ROOK][rook_rank][rook_file]  - eb->LatePST[ROOK][rook_rank][Move.target_file];
            Game->eval.mg_evaluation -= mg_eval_change;
            Game->eval.eg_evaluation -= eg_eval_change;
        }
    }
...
void make_move(Move played_move, struct GameState* Game, struct UndoInfo* Undo, struct Evalboards* eb){
    ...
    update_evaluation(Move, Game, eb, moving_piece, aPieces, ePieces, Undo);
